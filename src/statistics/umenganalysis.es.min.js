function buildUniqueId(t) {
    var n = [];
    n.push(DeviceUtil.getAdvertisingId()), n.push(DeviceUtil.getAndroidId()), Promise.all(n).then(n => {
        let e = n[0];
        let i = n[1];
        StorageUtil.putData(constants.MOBILE_ADVERTISING_ID, e);
        StorageUtil.putData(constants.MOBILE_ANDROID_ID, i);
        StorageUtil.putData("umeng_uniqueid", i || e);
        t(i ? i : e)
    })
}
    ...
    network.getType({
        success: function (n) {
            "none" != n.type && hasEnvelope(t, function (n, e) {
                ...
                if (n) {
                    if (e) {
                        console.log('==========hasEnvelope==========' , n , e );
                        const n = {
                            md5Key: e,
                            type: "second"
                        };
                        RequestHelper.getInstance().send(n, function () {
...
function hasEnvelope(t, n) {
    StorageUtil.getData(constants.KEY_NO_SEND_REQUEST_KEYS, function (e) {
        if (!e) return void n(!1, null);
        const i = UmengUtils.stringToArray(e);
        if (!i || 0 === i.length) return void n(!1, null);
        for (var o in i) {
            var s = i[o];
            if (s) {
                if (0 === s.indexOf(constants.KEY_EVENT_PREFIX) && t === constants.REQUEST_TYPE_EVENT) return void n(!0, s);
                if (0 === s.indexOf(constants.KEY_HALF_SESSION_PREFIX) && t === constants.REQUEST_TYPE_SESSION_HALF) return void n(!0, s);
                if (0 === s.indexOf(constants.KEY_CLOSE_SESSION_PREFIX) && t === constants.REQUEST_TYPE_SESSION_CLOSE) return void n(!0, s)
            }
        }
        n(!1, null)
    })
...
function realSend(t) {
    if (RequestHelper.getInstance().paramsIsValid(t))
        
        if (t === constants.REQUEST_TYPE_SESSION_HALF)(e = (n = RequestHelper.getInstance().buildSendContent(t, null)).data).analytics && RequestHelper.getInstance().send(n, function (t) {
            console.log('==============realSend 1 非首次======================');
        ...
        var n = RequestHelper.getInstance().buildSendContent(t, null);
        if (n && n.data) {
            var e = n.data;
            e.analytics && RequestHelper.getInstance().send(n)
        }
    } else t === constants.REQUEST_TYPE_EVENT && EventStoreQueue.getInstance().readEkvDatas(function (n) {
        console.log('==============realSend 2======================');
        // console.log( JSON.stringify( n ) );
        console.log('===============realSend 2=====================');
        if (n && 0 !== n.length) {
            var e = RequestHelper.getInstance().buildSendContent(t, n);
            e.data.analytics && RequestHelper.getInstance().send(e)
        }
    })
}

function PageController() {
    this._pagelist = [], this._currentPage = {}
}
...
function sendDatas() {
    ...
    console.log('============sendDatas========================');
    Header.getInstance().getHeader() ? 
    (
        Request.send(constants.REQUEST_TYPE_SESSION_HALF), 
        Request.send(constants.REQUEST_TYPE_SESSION_CLOSE), 
        Request.send(constants.REQUEST_TYPE_EVENT)
    ) : 
...
const StorageUtil = {
    ...
    constants = {
        ...
        TEST_URL: "http://hermesbeta.proxy.taobao.org/app_logs",
    ...
    network$1 = require("@system.network"),
    DeviceUtil = {
        getBaseInfo() {
...
const $JSON_UMENG_SESSIONS$ = "json_umeng_sessions",
    ...
    Session = function () {
        ...
        function n(t, n) {
            ...
            DeviceUtil.getUniqueId().then(t => {
                const i = UmengUtils.MD5(`${e}:::${Session.getInstance().appkey}:::${t}`);
                n(i)
            })
        }
        // 获取开始时间
        function e(t) {
            var n = {};
            return n.id = t, n.start_time = Date.now(), n
        }

        function i() {
            const t = Date.now();
            c ? r && t - r > constants.SESSION_INTERVAL && (o(r), s(c)) : StorageUtil.getData($JSON_UMENG_CURRENT_SESSION$, function (n) {
        ...
        function o(t) {
            ...
            let n = t - c.start_time;
            c.duration = n
            console.log('duration' , JSON.stringify(c) );
            
        }

        function s(t) {
            const i = t ? t.id : "",
                o = Session.getInstance().sessionListener(i);
            n(i, function (n) {
                c = e(n), StorageUtil.getData($JSON_UMENG_SESSIONS$, function (e) {
                    l = UmengUtils.stringToArray(e), t ? o.pages(function (e) {
                        e && e.length > 0 && (t.pages = e, t.$page_num = e.length), l.push(t), StorageUtil.putData($JSON_UMENG_SESSIONS$, UmengUtils.toString(l)), a(o, n)
                    }) : a(o, n)
                }), StorageUtil.putData($JSON_UMENG_CURRENT_SESSION$, UmengUtils.toString(c)), g = n
                console.log('UmengUtils.toString(c)' , UmengUtils.toString(c) );
        ...
        function a(t, n) {
            StorageUtil.getData($JSON_UMENG_HALF_SESSIONS$, function (e) {
                console.log('==============StorageUtil.getData======================');
                console.log( 'e' , e );
                console.log('===============StorageUtil.getData=====================');
                _ = UmengUtils.stringToArray(e), c && (_.push(c), StorageUtil.putData($JSON_UMENG_HALF_SESSIONS$, UmengUtils.toString(_)), t.generateSessionAfterHandler(n))
            })
        }
        let r, c, u = null,
            g = "",
    ...
    Header = function () {
        function t() {
            return {
                init(t) {
                    this.appkey = t, this.appInfo = App.getInfo();
                    const e = this;
                    n(function (t) {
                ...
                getHeader(t) {
                    switch (t) {
                        case constants.REQUEST_TYPE_SESSION_HALF:
                            this.header.st = "1", 1 == o && delete this.header.atm;
                            break;
        ...
        // 获取授权数据 ： 设备标识
        function n(t) {
            DeviceUtil.getBaseInfo().then(n => {
                DeviceUtil.getUniqueId().then(i => {
                    DeviceUtil.getNetworkInfo(function (o) {
                        let s = o.type;
                        s === MOBILE_NETWORK_NONE && (s = "unknown");
                        const a = e(n, i, s);
                        StorageUtil.getData($UMENG_KEY_IMPRINT$, function (n) {
                            0 != n.length && (a.imprint = n), 
                            StorageUtil.getData(constants.APP_FIRST_OPEN_FLAG, function (n) {
                                "true" === n && (a.atm = "1"), 
                                StorageUtil.getData(constants.MOBILE_ADVERTISING_ID, function (n) {
                                    a.advertising_id = n, 
        ...
        function e(t, n, e) {
            let o = null;
            if (t) {
                const e = t.screenWidth,
                    s = t.screenHeight,
                    a = e > s ? `${e}*${s}` : `${s}*${e}`,
                    r = i.appInfo;
                o = {
                    app_version: r.versionName,
                    ...
                    appkey: i.appkey,
                    device_model: t.model.toLowerCase().indexOf(t.brand.toLowerCase()) > -1 ? t.model : t.brand.toLowerCase() + BLANK_SPACE + t.model,
                    device_brand: t.brand,
                    channel: t.brand.toLowerCase(),
                    device_manufacturer: t.manufacturer,
                    device_manuid: t.model.toLowerCase().indexOf(t.brand.toLowerCase()) > -1 ? t.model : t.brand.toLowerCase() + BLANK_SPACE + t.model,
                    device_name: t.product.toLowerCase().indexOf(t.brand.toLowerCase()) > -1 ? t.product : t.brand.toLowerCase() + BLANK_SPACE + t.product,
                    os_version: t.osVersionName,
                    resolution: a,
                    language: t.language,
                    display_name: t.name,
                    idmd5: UmengUtils.MD5(n),
                    platform: "quickapp",
                    pro_ver: "1"
                }
            } else o = {
                app_version: i.appInfo.versionName,
                version_code: i.appInfo.versionCode,
                device_type: "Phone",
                package_name: i.appInfo.packageName,
                sdk_type: "Android",
                os: "Android",
                sdk_version: constants.SDK_VERSION,
                appkey: i.appkey,
                idmd5: UmengUtils.MD5(n),
                platfrom_source: "quickapp",
                pro_ver: "1"
            };
            switch (e) {
                ...
                case MOBILE_NETWORK_2G:
                    ...
                    break;
                default:
                    o.access = e, delete o.access_subtype
            }
            return o
        }
        let i = null,
...
Looper.prototype = {
    ...
    loop() {
        if (!this.isForceStop && this.queueObject && !this.isRunning)
            for (this.isRunning = !0; this.isRunning;) {
...
const RequestQueue = function () {
        ...
        function i(t) {
            for (let n = S.length - 1; n >= 0; n--) {
                ...
                if (e.key === t) {
                    if (!e.isExe) {
                        e.isExe = !0;
                        const t = e.timeOutFun;
                        t && clearTimeout(t), o(), S.splice(n, 1)
                    }
                    break
                }
            }
        ...
        function r() {
            return a(), {
                ...
                dispatchMessage() {
                    if (f > _) return void u.wait();
                    f++;
                    const t = n();
                    if (!s(t)) {
                        const n = t.executeFunction;
                        t.data ? e(t.md5Key, n) : RequestHelper.getInstance().send(t), this.isCanPop() || u.wait()
                    }
                },
                isCanPop() {
                    return g.length > 0 || l.length > 0
                }
            }
        }
        let c, u, g = [],
    ...
    StoreQueue = function () {
        ...
        function e() {
            S || (S = !0, StorageUtil.getData(constants.KEY_NO_SEND_REQUEST_KEYS, function (t) {
                if (S = !1, t) {
                    const n = UmengUtils.stringToArray(t);
                    for (let t in n) {
                        const e = n[t];
                        StorageUtil.getData(e, function (t) {
                            if (!t || '""' === t) {
                                const t = {
                                    key: e,
                                    type: f
        ...
        function a() {
            return e(), {
                ...
                dispatchMessage() {
                    if (E > l) return void c.wait();
                    E++;
                    let t = n();
                    if (!t || null === t) return void s();
                    const e = t.key;
                    t.type === _ ? i(e) : o(e), this.isCanPop() || c.wait()
                },
                isCanPop() {
...
const RequestHelper = function () {
        function t() {
            var t = [].concat(Session.getInstance().getSessions());
            if (t && 0 !== t.length) return Session.getInstance().clearSessions(), t
        }

        function n() {
            var t = [].concat(Session.getInstance().getHalfSessions());
            ...
            fetch.fetch({
                ...
                success: function (t) {
                    if (200 === t.code) {
                        e(n), o && o(n);
                        try {
                            let n = UmengUtils.toObject(t.data),
                                e = n.imprint;
                            StorageUtil.putData($UMENG_KEY_IMPRINT$$1, e), Header.getInstance().addHeaderAttr("imprint", e)
                        } catch (t) {}
                    }
                },
                fail: function (t, n) {},
        ...
        function o(t, n) {
            const e = UmengUtils.toString(n);
            var i = null;
            switch (t) {
                case constants.REQUEST_TYPE_SESSION_HALF:
                    i = constants.KEY_HALF_SESSION_PREFIX;
                    break;
                case constants.REQUEST_TYPE_SESSION_CLOSE:
                    i = constants.KEY_CLOSE_SESSION_PREFIX;
                    break;
                case constants.REQUEST_TYPE_EVENT:
                    i = constants.KEY_EVENT_PREFIX
            }
            const o = i + UmengUtils.MD5(e);
            return StorageUtil.putData(o, e), StoreQueue.getInstance().handleMessage(o), 0 === noSendRequestKeys.length && StorageUtil.getData(constants.KEY_NO_SEND_REQUEST_KEYS, function (t) {
                if (t) {
                    const n = UmengUtils.stringToArray(t);
                    0 === noSendRequestKeys.length && (noSendRequestKeys = n, r && r(noSendRequestKeys))
                }
            }), o
        }
        ...
        function s() {
            return {
                ...
                buildSendContent(e, i) {
                    var s = {
                        header: Header.getInstance().getHeader(e),
                        analytics: null
                    };
                    switch (e) {
                        case constants.REQUEST_TYPE_SESSION_HALF:
                        ...
                        case constants.REQUEST_TYPE_SESSION_CLOSE:
                            ...
                            break;
                        case constants.REQUEST_TYPE_EVENT:
                            i && i.length > 0 && (s.analytics = {
                                ekv: i
                            })
                    }
                    var c = "";
                    return s.analytics && (c = o(e, s)), {
                        data: s,
                        md5Key: c
                    }
                },
                send(t, n) {
                    let e = t.md5Key;
                    e && ("second" === t.type ? StorageUtil.getData(e, function (e) {
    ...
    EventStoreQueue = function () {
        ...
        function r(t, n) {
            let e, i = Session.getInstance().getCurrentSessionId(),
                o = -1;
            for (let n in t) {
                let s = t[n];
                if (e = s[i]) {
            ...
            return -1 === o && (e = []), e = e.concat(n), -1 === o ? t.push({
                [i]: e
            }) : t[o][i] = e, t
        }
            ...
            return {
                ...
                handlerNoSessionEkvDatas() {
                    if (U && U.length > 0) {
                        ...
                        for (let t in U) this.saveEkvData(U[t]);
                        U.splice(0, U.length)
                    }
                },
                quitHandler() {
                    0 !== h.length && (S.forceStop(), E <= f ? StorageUtil.getData(g, function (t) {
                        let n = r(UmengUtils.stringToArray(t), h);
                        StorageUtil.putData(g, UmengUtils.toString(n)), h.splice(0, h.length), S.cancelForceStop()
                    }, function () {
                        S.cancelForceStop()
                    }) : o("quitHandler", function (t) {
                        let n = UmengUtils.toString(r(t, h));
                        StorageUtil.putData(g, n), h.splice(0, h.length), s("quitHandler"), S.cancelForceStop()
                    }))
                },
                dispatchMessage() {
                    if (E > f) return void S.wait();
                    E++;
                    const t = n();
                    if (!t || 0 === t.length) return void e();
                    StorageUtil.getData(u, function (n) {
                        n || (n = 0), n >= l + t.length ? e() : StorageUtil.getData(g, function (e) {
...
const EventController = function () {
        ...
        function n() {
            return {
                addEvent(n, e) {
                    var i = t(n, e);
                    EventStoreQueue.getInstance().saveEkvData(i)
                },
                handlerNoSessionEkvDatas() {
...
PageController.prototype = {
    ...
    savePage(t, n) {
        let e = n.$page.uri;
        if (t && e === this._currentPage.page_name) {
            var i = Date.now() - this._currentPage.ts,
                o = "/";
            try {
                if (o = e.split(Header.getInstance().getPackageName())[1], "/" === (o = o.split("?")[0]) && n.$app && n.$app.entry) {
                    let t = n.$app.entry;
                    o += t
                }
            } catch (t) {}
            this._currentPage.page_name = o, this._currentPage.duration = i, this._pagelist = this._pagelist.concat(this._currentPage), this._currentPage = {}, this.saveStroragePages(t)
        }
    },
    // 更新缓存的页面相关信息
    saveStroragePages(t) {
        console.log('更新缓存的页面相关信息' , JSON.stringify( this._pagelist ));
    ...
    getPages(t, n) {
        ...
        let e = this;
        StorageUtil.getData(KEY_PRE_PAGE + t, function (i) {
            var o = UmengUtils.stringToArray(i).concat(e._pagelist);
            e.clear(t), n && n(o)
        }, function (t, e) {
            n && n(null)
        })
    ...
    updateStorage(t, n) {
        if (n && 0 !== n.length) {
            let e = this;
            StorageUtil.getData(t, function (i) {
                var o = UmengUtils.stringToArray(i).concat(n);
                StorageUtil.putData(t, UmengUtils.toString(o), function (t) {
                    e._pagelist = []
                })
            })
        }
...
UmengAnalysisLib.prototype = {
    ...
    init(t) {
        ...
        if (!inited) {
            ...
            Session.getInstance().setOnSessionListener(function (t) {
    ...
    // 记录页面打开
    resume(t) {
        console.log('打开页面' , JSON.stringify( t.$page.uri ) );
        ...
        if (isInit() && t) {
            ...
            this.pageController.addCurrentPage(n), Session.getInstance().init(this.appkey)
        }
    },
    // 记录页面关闭
    pause(t) {
        console.log('关闭页面' , JSON.stringify( t.$page ) );
...
const umeng_stat_global = Object.getPrototypeOf(global) || global,
    UmengAnalysis = {
        _analysisLib: new UmengAnalysisLib,
        init(t) {
